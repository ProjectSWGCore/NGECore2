...
public class CollectionServerFirstListResponse extends SWGMessage {
	...
	public CollectionServerFirstListResponse(Map<String, ServerFirst> serverFirstList){
	...
	public IoBuffer serialize() {
		...
		synchronized (sfList) {
			String galaxy = NGECore.getInstance().getConfig().getString("GALAXY_NAME");
			buffer = IoBuffer.allocate(14 + galaxy.length()).order(ByteOrder.LITTLE_ENDIAN);
			buffer.putShort((short) 2);
			buffer.putInt(CRC.StringtoCRC("CollectionServerFirstListResponse"));
			
			buffer.put(getAsciiString(galaxy + ".0"));
			buffer.putInt(sfList.size());
			
			for (Entry<String, ServerFirst> entry : sfList.entrySet()) {
				buffer.expand(entry.getValue().getBytes().length);
