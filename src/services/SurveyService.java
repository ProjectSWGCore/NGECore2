...
public class SurveyService implements INetworkDispatch {
	...
	private void processActiveTools() {
		synchronized (activeSurveyTools) {
			// All tools sampling
			Iterator <SurveyTool> surveyIterator = activeSurveyTools.iterator();
			while (surveyIterator.hasNext()) {
				SurveyTool surveyTool = surveyIterator.next();
				if (surveyTool == null)
					continue;
	...
	@SuppressWarnings("unused")
	private void handleSamplingStages(SurveyTool surveyTool) {
		...
		playerInventory.viewChildren(owner, false, false, new Traverser() {
			...
			public void process(SWGObject obj) {
				if (obj instanceof ResourceContainerObject) {
					...
					if (container.getReferenceID() == resource.getId() && container.getStackCount() < ResourceContainerObject.maximalStackCapacity - 10)
						found.add(container);
				}
			}
		});
...
//		}
		if (target == null) {
			System.err.println("SurveyService: Target Object Not Valid for surveying.");
			return; // target object not valid
		}
			
		if (crafter.getContainer() != null) {
			System.err.println("SurveyService: Container is not null!");
			return;
		}
		
		SurveyTool surveyTool = (SurveyTool) target;
		if (surveyTool.getToolType() == -1) {
			System.err.println("SurveyService: Survey tool type was not recognized.");
		...
		if (surveyTool.getCurrentlySurveying()) {
			System.err.println("SurveyService: Pressed survey button too frequently");
			return;
		}
...
//		core.surveyService.addObserver(toolObserver);
		...
		synchronized (activeSurveyTools) {
			if (!activeSurveyTools.contains(surveyTool))
				activeSurveyTools.add(surveyTool);
		}
	...
	public boolean toolIsInList(SurveyTool surveyTool) {
		synchronized (activeSurveyTools) {
			for (SurveyTool tool : activeSurveyTools) {
