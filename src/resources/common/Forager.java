...
public class Forager {
	...
	public static final String[] RARE_COMPONENTS_CORELLIA = new String[] {
			"object/tangible/loot/creature_loot/collections/fried_icecream_components/corellia/shared_charbote.iff",
	...
	public static final String[] RARE_COMPONENTS_DANTOOINE = new String[] {
			"object/tangible/loot/creature_loot/collections/fried_icecream_components/dantooine/shared_barabel.iff",
	...
	public static final String[] RARE_COMPONENTS_DATHOMIR = new String[] {
			"object/tangible/loot/creature_loot/collections/fried_icecream_components/dathomir/shared_beebleberry.iff",
	...
	public static final String[] RARE_COMPONENTS_ENDOR = new String[] {
			"object/tangible/loot/creature_loot/collections/fried_icecream_components/endor/shared_acorns.iff",
	...
	public static final String[] RARE_COMPONENTS_NABOO = new String[] {
			"object/tangible/loot/creature_loot/collections/fried_icecream_components/naboo/shared_cligs.iff",
	...
	public static final String[] RARE_COMPONENTS_TATOOINE = new String[] {
			"object/tangible/loot/creature_loot/collections/fried_icecream_components/tatooine/shared_banthaweed.iff",
	...
	public static final String[] RARE_COMPONENTS_YAVIN = new String[] {
			"object/tangible/loot/creature_loot/collections/fried_icecream_components/yavin/shared_blueleaf_shrub.iff",
	...
	public boolean handleForageResults(CreatureObject forager) {
		...
		int[] forageChances2 = new int[] { 12, 10, 21, 23, 24, 10 }; // Kommerken
																		// Steak
		int[] forageChances3 = new int[] { 20, 9, 22, 22, 22, 5 }; // if you got
																	// the pig
		...
		// }
		// forager.sendSystemMessage("randomForageRoll " +
		// randomForageRoll,(byte) 0);
		if (randomForageRoll < forageResultChance + kommerkenBonus) {
			...
			if (forager.getInventoryItemCount() >= 80) {
				chosenObject = 6;
				forager.sendSystemMessage("@skl_use:sys_forage_noroom",
						(byte) 0);
			}

			// chosenObject = 5; // To test TMs
			if (!(NGECore.getInstance().adminService
					.getAccessLevelFromDB(forager.getClient().getAccountId()) == null)) // Always
																						// yield
																						// a
																						// TM
																						// for
			...
			// forager.sendSystemMessage("chosenObject " + chosenObject,(byte)
			// 0);
			switch (chosenObject) {

			case LYASE_ENZYME:
				String template = "object/tangible/loot/beast/shared_enzyme_2.iff";
				TangibleObject foragedObject = (TangibleObject) NGECore
						.getInstance().objectService.createObject(template,
						forager.getPlanet());
				// determine color here
				foragedObject.setCustomizationVariable(
						"/private/index_color_1",
						(byte) new Random().nextInt(10)); // edb_vial.pal
			...
			case RARE_COMPONENT:
				template = "";
				String[] componentArray = rareComponentHash.get(forager
						.getPlanetId());
				int componentRoll = new Random().nextInt(componentArray.length);
				template = componentArray[componentRoll];
				foragedObject = (TangibleObject) NGECore.getInstance().objectService
						.createObject(template, forager.getPlanet());
				foragerInventory.add(foragedObject);
				forager.sendSystemMessage("", (byte) 0);
				break;

			case INSECT_BAIT:
				template = "object/tangible/fishing/bait/shared_bait_insect.iff";
				foragedObject = (TangibleObject) NGECore.getInstance().objectService
						.createObject(template, forager.getPlanet());
				foragerInventory.add(foragedObject);
				forager.sendSystemMessage("", (byte) 0);
			...
			case GRUB_BAIT:
				template = "object/tangible/fishing/bait/shared_bait_grub.iff";
				foragedObject = (TangibleObject) NGECore.getInstance().objectService
						.createObject(template, forager.getPlanet());
				foragerInventory.add(foragedObject);
				forager.sendSystemMessage("", (byte) 0);
			...
			case WORM_BAIT:
				template = "object/tangible/fishing/bait/shared_bait_worm.iff";
				foragedObject = (TangibleObject) NGECore.getInstance().objectService
						.createObject(template, forager.getPlanet());
				foragerInventory.add(foragedObject);
				forager.sendSystemMessage("", (byte) 0);
			...
			case MYSTERIOUS_DATA_DISC:
				template = "object/tangible/loot/generic_usable/shared_datadisk_generic.iff";
				foragedObject = (TangibleObject) NGECore.getInstance().objectService
						.createObject(template, forager.getPlanet());
				foragedObject.setCustomName(getLevelDependentMapName(forager
						.getLevel()));
				foragedObject.setAttachment("MapLevel", forager.getLevel());
				foragedObject.setAttachment("MapSTFName",
						getLevelDependentSTFName(forager.getLevel()));
				int mapPlanetID = new Random().nextInt(mapPlanetIDs.length - 1);
				foragedObject.setAttachment("MapPlanet",
						mapPlanetIDs[mapPlanetID]);
				// System.out.println("mapPlanetID " +
				// mapPlanetIDs[mapPlanetID]);
				foragedObject.setAttachment("radial_filename",
						"object/treasuremap");
				// foragedObject.setAttachment("radial_filename",
				// "object/treasuremapExtract");
				foragerInventory.add(foragedObject);
				forager.sendSystemMessage("", (byte) 0);
				break;
			...
			default:
				break;
			// /object/tangible/treasure_map/shared_treasure_map_base.iff
			// shared_treasure_map_quest.iff
			}

			if (chosenObject < 6)
				forager.sendSystemMessage("@skl_use:sys_forage_success",
						(byte) 0);

			// spawn adversaries
			int adversaryRoll = new Random().nextInt(100);
			// A new buff is imaginable that would decrease the chance of
			// running into adversaries here, just an idea
	...
	public void spawnWormie(CreatureObject forager) {
		...
		String name = "";
		Point3D wormiePosition = SpawnPoint.getRandomPosition(
				forager.getPosition(), 1, 6, forager.getPlanetId());
		CreatureObject wormie = NGECore.getInstance().spawnService
				.spawnCreature("forage_worm", forager.getPlanet().getName(),
						0L, wormiePosition.x, wormiePosition.y,
						wormiePosition.z, (short) level);
	...
	public void spawnGoon(CreatureObject forager) {
		...
		Point3D wormiePosition = SpawnPoint.getRandomPosition(
				forager.getPosition(), 1, 6, forager.getPlanetId());
		CreatureObject goon = NGECore.getInstance().spawnService.spawnCreature(
				"forage_goon", forager.getPlanet().getName(), 0L,
				wormiePosition.x, wormiePosition.y, wormiePosition.z,
				(short) level);
		...
		actor.addDefender(forager);
		forager.sendSystemMessage("@foraging/forage_enemy:" + barkString,
				DisplayType.Broadcast);
		OutOfBand oob = OutOfBand.ProsePackage("@foraging/forage_enemy:"
				+ barkString);
		Stf stf = new Stf("@foraging/forage_enemy:" + barkString);
		NGECore.getInstance().chatService.spatialChat(goon, forager,
				stf.getStfValue(), (short) 0x0, (short) 0x0, 1, oob);
	}
	...
	public void handleGuardSpawn(CreatureObject owner, TangibleObject map) {
		...
		// Point3D exactTreasureLocation = (Point3D)
		// map.getAttachment("MapExactLocation");
		Point3D exactTreasureLocation = owner.getPosition();

		GroupObject extractorGroup = null;
		if (owner.getGroupId() != 0)
			extractorGroup = (GroupObject) NGECore.getInstance().objectService
					.getObject(owner.getGroupId());
		...
		for (int i = 0; i < guardSpawnNumber; i++) {
			guardList.add(spawnGuard(exactTreasureLocation, owner, map,
					(short) (spawnLevel + 2 - 80)));
		}
		...
		if (extractorGroupSize == 8) {
			...
			if (bossRoll < 70) {
				guardList.add(spawnBoss(exactTreasureLocation, owner, map,
						(short) (spawnLevel + 2)));
		...
		TangibleObject treasureContainer = (TangibleObject) NGECore
				.getInstance().staticService.spawnObject(
				"object/tangible/container/drum/shared_treasure_drum.iff",
				owner.getPlanet().getName(), 0L, exactTreasureLocation.x,
				exactTreasureLocation.y, exactTreasureLocation.z, 0.70F, 0.71F);

		// CreatureObject treasureContainer = (CreatureObject)
		// NGECore.getInstance().objectService.createObject("object/tangible/container/drum/shared_treasure_drum.iff",
		// 0, owner.getPlanet(), exactTreasureLocation, owner.getOrientation());
		// NGECore.getInstance().simulationService.add(treasureContainer,
		// exactTreasureLocation.x, exactTreasureLocation.z, true);

		treasureContainer.setAttachment("radial_filename",
				"object/treasureContainer");
		treasureContainer.setAttachment("TreasureExtractorID",
				owner.getObjectID());
		treasureContainer.setAttachment("TreasureGuards", guardList);
		// treasureContainer.setAttachment("ChestLevel",new
		// Integer(spawnLevel));
		configureTreasureLoot(treasureContainer, owner, spawnLevel);
		NGECore.getInstance().lootService.DropLoot(owner, treasureContainer);

		owner.sendSystemMessage("@treasure_map/treasure_map:sys_time_limit",
				(byte) 0);
			...
			public void run() {
				try {
					...
					owner.sendSystemMessage(
							"@treasure_map/treasure_map:unlock_chest", (byte) 0);
					Thread.currentThread().interrupt();
	...
	public CreatureObject spawnGuard(Point3D exactTreasureLocation,
			CreatureObject owner, TangibleObject map, short spawnLevel) {

		String name = "";
		// System.out.println("exactTreasureLocation.x + "
		// +exactTreasureLocation.x);
		// System.out.println("exactTreasureLocation.y + "
		// +exactTreasureLocation.y);
		// System.out.println("exactTreasureLocation.z + "
		// +exactTreasureLocation.z);

		Point3D guardPosition = SpawnPoint.getRandomPosition(
				exactTreasureLocation, 5, 12, owner.getPlanetId());
		CreatureObject guard = NGECore.getInstance().spawnService
				.spawnCreature("treasure_guard", owner.getPlanet().getName(),
						0L, guardPosition.x, guardPosition.y, guardPosition.z,
						spawnLevel);
		...
		owner.sendSystemMessage("@treasure_map/treasure_map:" + barkString,
				DisplayType.Broadcast);
		OutOfBand oob = OutOfBand.ProsePackage("@treasure_map/treasure_map:"
				+ barkString);
		NGECore.getInstance().chatService.spatialChat(guard, owner, "",
				(short) 0x0, (short) 0x0, 1, oob);
		return guard;
	}

	public CreatureObject spawnBoss(Point3D exactTreasureLocation,
			CreatureObject owner, TangibleObject map, short spawnLevel) {

		// Point3D treasureLocation =
		// resources.common.SpawnPoint.getRandomPosition(exactTreasureLocation,
		// 1, 10, owner.getPlanetId());
		return new CreatureObject(); // FIXME on paper this looks bad
	}

	@SuppressWarnings("unchecked")
	public int countAliveGuards(SWGObject treasureContainer) {
		Vector<CreatureObject> guardList = (Vector<CreatureObject>) treasureContainer
				.getAttachment("TreasureGuards");
		int aliveCount = 0;
	...
	public void configureTreasureLoot(TangibleObject treasureContainer,
			CreatureObject owner, short spawnLevel) {
		@SuppressWarnings("unused")
		...
		// String[] lootPoolNames = new
		// String[]{"profession_bracelets_"+levelRange,"house_furniture","wearable_backpacks"};
		// double[] lootPoolChances = new double[]{60,30,10};
		String[] lootPoolNames = new String[] { "profession_bracelets_"
				+ levelRange };
		double[] lootPoolChances = new double[] { 100 };
		double lootGroupChance = 80;
		treasureContainer.addToLootGroups(lootPoolNames, lootPoolChances,
				lootGroupChance);

		// WEAPONSMITH COMPONENTS JEDI COMPONENTS GEM COLLECTION ITEMS TREASURE
		// HUNTER COLLECTION ITEMS
		// lootPoolNames = new
		// String[]{"weaponsmith_components","jedi_components","gem_collection_items","treasure_hunter_collection_items"};
		...
		// COMMON LOOTS:
		// Looted non-wearable Bounty Hunter armor pieces (Used to make
		// Mandalorian armor at the Death Watch Bunker)
		// Looted clothing (usually 20+)
		// Looted armors (usually 20+)
		// Looted weapons (700-800 DPS(Sometimes less, now includes the Nak'tra
		// Crystal Rifle))
		...
		// Various Schematics
		// lootPoolNames = new
		// String[]{"bh_armor_for_mando","advanced_stims_80","loot_kits_adhesives","various_schematics"};
		// lootPoolChances = new double[]{15,25,30,30};
		// lootGroupChance = 80;
		// treasureContainer.addToLootGroups(lootPoolNames, lootPoolChances,
		// lootGroupChance);
